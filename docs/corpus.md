...
src/corpus/passes/           structure.ts, search.ts, index.ts, fts.ts, tld-links.ts, refs.ts, morph.ts, freq.ts (one per table group)
...
scripts/freq/                the counting pipeline behind corpus/freq/counts.tsv (bun run freq:fetch / freq:count / freq:classify / freq:reduce)
```
...
| `full` (default) | `structure`, `search`, `index`, `fts`, `tld-links`, `refs`, `morph`, `freq` | 293.0 MB | 135.9 MB | every tool |
...
tables, `morph` reads `x_tld_occ`, and `freq` reads `x_morph` and `x_token`. Rows at `d18ad4f`:
...
| `freq` | `x_freq_word`, `x_freq_root` | 227,920 |
