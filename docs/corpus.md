...
pnpm corpus:build               # XML → data/voko.db, then all passes (~4 min, ~280 MB, + voko.db.gz)
pnpm corpus:build --stage core  # what a browser downloads: articles + structure + search + morph (~143 MB, ~63 MB gzipped)
...
src/corpus/passes/           structure.ts, search.ts, index.ts, fts.ts, tld-links.ts, refs.ts, morph.ts, freq.ts (one per table group; morph.ts holds `morph` and `splits`)
...
src/freq.ts                  runtime-safe reads of the frequency tables: wordFrequency(), morphFrequency()
...
| `full` (default) | `structure`, `search`, `morph`, `index`, `fts`, `tld-links`, `refs`, `splits`, `freq` | 292.9 MB | 137.6 MB | every tool |
...
reads `x_morph` and `x_token`, so it runs after `splits`. Rows:
