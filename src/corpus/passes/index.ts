 ...
 * (the `structure` pass's node(mrk)). The thesaurus finds a word's nodes by
 * any headword spelling and walks down to the senses, gloss matches
 * translations by their index form; those indexes are built here, for a
 * database that serves those tools, and are not shipped to browsers.
 */
import type { Pass } from "../pass";

const INDEXES = [
  "CREATE INDEX IF NOT EXISTS idx_headword_node ON headword(node_id)",
  "CREATE INDEX IF NOT EXISTS idx_headword_norm ON headword(norm)",
